...
scprint_spatial fit --config config/0510_base_spatial_multiscale_medium.yml --scprint_training.name ${DATE}_base_spatial_multiscale_weight${COMBINE_WEIGHT}_${NODE_TYPE} --model.combine_weight ${COMBINE_WEIGHT} --model.image_combine_weight=${IMAGE_COMBINE_WEIGHT} --model.image_recon_loss_weight=${IMAGE_RECON_LOSS_WEIGHT} --data.batch_size 224 --model.ckpt_path data/model/scPRINT/medium.ckpt --data.collection_name xenium_multiscale_v7_1024 --data.num_workers 8
