...
def eval_model(
    ...
    trt2ctrl_idx = {}
    if hasattr(datamodule, 'mol2id') and isinstance(datamodule.mol2id, dict):
    ...
    for data_iter_step, batch in tqdm(enumerate(data_loader)):
        ...
        if spatia_conditioner is not None:
            delta_g = batch.get('delta_g')
            delta_m = batch.get('delta_m')
            transition_ids = batch.get('transition_ids')
            if delta_g is not None and delta_m is not None and transition_ids is not None:
                delta_g = delta_g.to(device)
                delta_m = delta_m.to(device)
                transition_ids = transition_ids.to(device)
                with torch.no_grad():
        ...
        if num_synthetic < fid_samples:
            ...
            if 'file_names' in batch:
                img_file_ctrl, img_file_trt = batch['file_names']
            else:
                img_file_ctrl = [f"ctrl_{i}" for i in range(x_real_ctrl.shape[0])]
                img_file_trt = [f"tgt_{i}" for i in range(x_real_trt.shape[0])]
            if args.save_fid_samples and args.output_dir:
                ...
                for batch_index, image_np in enumerate(images_np):
                    ...
                    class_id = int(np.asarray(target_class_labels[batch_index]).reshape(-1)[0])
                    if id2mol and class_id in id2mol:
                        target_class_name = id2mol[class_id]
                    else:
                        target_class_name = f"class_{class_id}"
