...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Override: BBBC021 image folder (defaults to configs/<dataset>.yaml)',
    )
    parser.add_argument(
        '--data_index_path',
        type=str,
        default=None,
        help='Override: metadata CSV (e.g. bbbc021_df_all.csv)',
    )
    parser.add_argument(
        '--embedding_path',
        type=str,
        default=None,
        help='Override: compound embedding CSV (emb_fp_all.csv)',
    )
    ...
    if cli_args.image_path:
        args.image_path = cli_args.image_path
    if cli_args.data_index_path:
        args.data_index_path = cli_args.data_index_path
    if cli_args.embedding_path:
        args.embedding_path = cli_args.embedding_path
    args.batch_size = 32
    ...
    for batch in tqdm(data_loader_test):
        ...
        for i in range(real_samples.shape[0]):
            ...
            candidates = [
                os.path.join(synthetic_samples_path, target_class, f'{img_file_trt[i]}.png'),
                os.path.join(synthetic_samples_path, target_class, f'{int(idx_trt[i])}.png'),
            ]
            loaded = False
            for path in candidates:
                if os.path.exists(path):
                    synthetic_samples.append(read_img_from_path(path))
        ...
        if len(synthetic_samples) == 0:
            continue
        real_samples = real_samples[valid_indices]
        target_classes = [target_classes[i] for i in valid_indices]
